...
public class RoomDao {
    ...
    /** 更新客房状态（使用外部连接，用于事务） */
    public int updateStatus(Connection conn, Integer id, String status) throws SQLException {
        String sql = "UPDATE room SET status = ? WHERE id = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, status);
            stmt.setInt(2, id);
            int result = stmt.executeUpdate();
            LogUtil.info("更新客房状态成功(事务): ID=" + id + ", 状态=" + status);
            return result;
        }
    }

    /** 根据ID查询客房（使用外部连接，用于事务） */
    public Room findById(Connection conn, Integer id) throws SQLException {
        String sql = "SELECT * FROM room WHERE id = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, id);
            try (ResultSet rs = stmt.executeQuery()) {
